...
def get_association(word: str, category: str = '', pos: str = '') -> dict:
    temp = association_dict(word=word, category=category, pos=pos)
    temp['after'] = set([WNL.lemmatize(x) for x in temp['after']])
    temp['before'] = set([WNL.lemmatize(x) for x in temp['before']])
    temp['trigger'] = set([WNL.lemmatize(x) for x in temp['trigger']])
    return temp
...
def before(word: str, category: str = '', pos: str = '') -> list:
    ...
    words = [x if pos else x['word']
             for x in API.words(rel_bgb=category, ml=word, md='p') if 'tags' in x]
...
def after(word: str, category: str = '', pos: str = '') -> list:
    ...
    """
    words = [x if pos else x['word']
             for x in API.words(rel_bga=category, ml=word, md='p') if 'tags' in x]
    return [x['word'] for x in words if pos in x['tags']] if pos else words
...
def trigger(word: str, category: str = '', pos: str = '') -> list:
    ...
    """
    words = [x if pos else x['word']
             for x in API.words(rel_trg=category, ml=word, md='p') if 'tags' in x]
    return [x['word'] for x in words if pos in x['tags']] if pos else words
